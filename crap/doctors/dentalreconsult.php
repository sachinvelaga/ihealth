...
$sql_check_user = "SELECT * FROM `dental_report` WHERE `p_id`='".$personid."' AND `comment_status` = '2' ";
...
$htm_view .='<tr style="width:100%; border:1px solid #CCC;text-align:center;">
  ...
  if($result['comment']==''||$result['comment']==null){
	  ...
	  $htm_view .='<td style="width:25%; border:1px solid #CCC;"><input type="hidden" value="update" id="cmnttype_'.$result['d_id'].'"><a href="javascript:" id="commentlink_'.$result['d_id'].'" onclick="commentupdate('.$result['d_id'].')">Update</a></td></tr>';
	  }
$count++;
}
	}else{

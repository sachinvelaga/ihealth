...
$htm_view .='<tr style="width:100%; border:1px solid #CCC;text-align:center;">
  ...
  if($result['doctor_comments']==''||$result['doctor_comments']==null){
	  ...
	  $htm_view .='<td style="width:10%; border:1px solid #CCC;"><input type="hidden" value="update" id="updatetype_'.$result['b_id'].'"><a href="javascript:" id="ommentlink_'.$result['b_id'].'" onclick="commentupdate('.$result['b_id'].')">Update</a></br><input type="hidden" value="reconsult" id="cmnttype_'.$result['b_id'].'"><a href="javascript:" id="reconsultlink_'.$result['b_id'].'" onclick="commentreconsult('.$result['b_id'].')">Reconsult</a></td></tr>';
	  }
$count++;
}
	}else{

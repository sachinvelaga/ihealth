...
$sql_check_user = "SELECT * FROM `hearing_tbl` WHERE `p_id`='".$personid."' AND `comment_status` = '2' ";
...
$htm_view .='<tr style="width:100%; border:1px solid #CCC;text-align:center;">
  ...
  if($result['doctor_comments']==''||$result['doctor_comments']==null){
	  ...
	  $htm_view .='<td style="width:10%; border:1px solid #CCC;"><input type="hidden" value="update" id="cmnttype_'.$result['h_id'].'"><a href="javascript:" id="commentlink_'.$result['h_id'].'" onclick="commentupdate('.$result['h_id'].')">Update</a></td></tr>';
	  }

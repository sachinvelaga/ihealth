...
$sql_check_user = "SELECT * FROM `fitness_tbl` WHERE `s_id`='".$personid."' AND `comment_status` = '0' ";
...
$htm_view .='<tr style="width:100%; border:1px solid #CCC;text-align:center;">
  ...
  if($result['doctor_comment']==''||$result['doctor_comment']==null){
	  ...
	  $htm_view .='<td style="width:7%; border:1px solid #CCC;"><input type="hidden" value="update" id="updatetype_'.$result['f_id'].'"><a href="javascript:" id="commentlink_'.$result['f_id'].'" onclick="commentupdate('.$result['f_id'].')">Update</a></br><input type="hidden" value="reconsult" id="cmnttype_'.$result['f_id'].'"><a href="javascript:" id="reconsultlink_'.$result['f_id'].'" onclick="commentreconsult('.$result['f_id'].')">Reconsult</a></td></tr>';
	  }
$count++;
}
	}else{
